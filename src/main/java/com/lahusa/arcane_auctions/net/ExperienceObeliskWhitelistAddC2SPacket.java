...
public class ExperienceObeliskWhitelistAddC2SPacket {
    public BlockPos pos;
    public String username;

    public ExperienceObeliskWhitelistAddC2SPacket(BlockPos pos, String username) {
        this.pos = pos;
        this.username = username;
    }

    public ExperienceObeliskWhitelistAddC2SPacket(FriendlyByteBuf buf) {
    ...
    public boolean handle(Supplier<NetworkEvent.Context> ctx) {
        ...
        ctx.get().enqueueWork(() -> {
            ...
            if(serverEntity instanceof ExperienceObeliskBlockEntity obeliskEntity){
                boolean transactionSuccess = obeliskEntity.handleWhitelistAdd(username, sender);
